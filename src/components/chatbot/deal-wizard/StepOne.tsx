    ...
    <div className="space-y-6">
        ...
        <div className="relative">
          <select
            ...
            className={`
              ...
              disabled:bg-gray-50 disabled:text-gray-500 dark:disabled:bg-dark-bg dark:disabled:text-dark-text-secondary
              dark:bg-dark-bg dark:border-dark-border dark:text-dark-text
              ${lockedFields ? 'bg-gray-100 dark:bg-dark-bg cursor-not-allowed' : ''}
              ${errors.requisitionId ? 'border-red-300 bg-red-50' : 'border-gray-300 dark:border-dark-border'}
            `}
          >
          ...
          <div className="mt-3 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-lg">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 text-sm">
                <p className="font-medium text-blue-900 dark:text-blue-200">{selectedRfq.title}</p>
      ...
      <div>
        ...
        {selectedVendorNames && selectedVendorNames.length > 0 && lockedFields ? (
          <div className="w-full px-4 py-2.5 border border-gray-300 dark:border-dark-border rounded-lg bg-gray-100 dark:bg-dark-bg">
          ...
          <div className={`border rounded-lg max-h-48 overflow-y-auto divide-y divide-gray-100 dark:divide-dark-border ${errors.vendorId ? 'border-red-300 bg-red-50' : 'border-gray-300 dark:border-dark-border'}`}>
            {loadingVendors ? (
              <div className="flex items-center justify-center gap-2 p-4 text-sm text-gray-500 dark:text-dark-text-secondary">
                <Loader2 className="w-4 h-4 animate-spin text-gray-400 dark:text-gray-500" />
                Loading vendors...
              </div>
            ) : !data.requisitionId ? (
              <p className="p-3 text-sm text-gray-500 dark:text-dark-text-secondary">Select an RFQ first...</p>
            ) : safeVendors.length === 0 ? (
              <p className="p-3 text-sm text-gray-500 dark:text-dark-text-secondary">No vendors attached to this RFQ</p>
            ) : (
              safeVendors.map(vendor => {
                const isChecked = (data.vendorIds ?? []).includes(vendor.id);
                return (
                  <label key={vendor.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={isChecked}
                      onChange={() => handleVendorToggle(vendor.id)}
                      className="w-4 h-4 text-blue-600 rounded border-gray-300 dark:border-dark-border focus:ring-blue-500"
                    />
                    <span className="text-sm font-medium text-gray-900 dark:text-dark-text">{vendor.name}</span>
                    {vendor.companyName && <span className="text-xs text-gray-500 dark:text-dark-text-secondary">({vendor.companyName})</span>}
                    {vendor.pastDealsCount > 0 && (
                      <span className="ml-auto text-xs text-gray-400 dark:text-gray-500">{vendor.pastDealsCount} past deals</span>
                    )}
                  </label>
                );
              })
          ...
          <div className="relative">
            <select
              ...
              className={`
                ...
                disabled:bg-gray-50 disabled:text-gray-500 dark:disabled:bg-dark-bg dark:disabled:text-dark-text-secondary
                dark:bg-dark-bg dark:border-dark-border dark:text-dark-text
                ${(lockedFields || data.vendorLocked) ? 'bg-gray-100 dark:bg-dark-bg cursor-not-allowed' : ''}
                ${errors.vendorId ? 'border-red-300 bg-red-50' : 'border-gray-300 dark:border-dark-border'}
              `}
            >
              <option value="">
                {!data.requisitionId
      ...
      <div>
        ...
        <input
          ...
          className={`
            ...
            focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent
            dark:bg-dark-bg dark:border-dark-border dark:text-dark-text
            ${errors.title ? 'border-red-300 bg-red-50' : 'border-gray-300 dark:border-dark-border'}
          `}
        />
        {errors.title && (
          <p className="mt-1 text-sm text-red-600 flex items-center gap-1">
      ...
      <div>
        ...
        <div className="w-full px-4 py-2.5 bg-gray-50 dark:bg-dark-bg border border-gray-300 dark:border-dark-border rounded-lg text-gray-700 dark:text-dark-text font-medium">
          CONVERSATION (AI Conversational Negotiation)
        </div>
        <p className="mt-1 text-xs text-gray-500 dark:text-dark-text-secondary">
          AI-assisted conversational negotiation with supplier
        </p>
      </div>
