...
export default function OfferCard({ offer, currency = 'USD' }: OfferCardProps) {
  ...
  const activeCurrency = (offer as any).currency || (offer as any).currencyCode || currency || 'USD';
  const hasPrice = price !== null && price !== undefined && !isNaN(Number(price));
  const hasTerms = typeof offer.payment_terms === 'string' && offer.payment_terms.trim().length > 0;
  const deliveryText = formatDelivery(offer.delivery_date, offer.delivery_days);
  const hasDelivery = typeof deliveryText === 'string' && deliveryText.length > 0;
  ...
  return (
    <div className="flex items-center gap-2 px-3 pt-1 pb-0.5 bg-gray-50 border border-gray-200 rounded text-sm">
      ...
      {hasPrice && (
        <span className="text-gray-900 font-semibold">{formatOfferPrice(Number(price), activeCurrency)}</span>
      )}
